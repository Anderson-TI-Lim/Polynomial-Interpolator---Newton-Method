let deltaN = [], parametersFake
let round = 0, variableGuard = [], end = 0

function turningDeltaIntoPolynomial(delta, parametersOriginal) {
    let firstKeeper = [], multiplicationKeeper = [], polynomial = ``
...
function main(parametersOriginal) {
    parametersFake = parametersOriginal ,parametersFake.xFake = []
    while (true) {
        if (end === 1) {
            break
        } else {
            ...
            if (deltaN.length === 1) {
                for (let counter = 0; counter < parametersFake.fX.length - 1; counter++) {
                    ...
                    })), round++, variableGuard.push(parametersFake.x[counter + 1], parametersFake.x[counter])
